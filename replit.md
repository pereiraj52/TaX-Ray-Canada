...
- July 1, 2025. Added additional charitable donation fields: included Allowable Charitable Donations (Line 34000) and Total Ecological Gifts (Line 34200) in FederalTaxFields dataclass, added field mappings to Python extractor and PDF parser, updated frontend ExtractedDataDisplay to show both new fields in Charitable Gifts & Donations section, and integrated both fields into Tax Credits Analysis for complete charitable giving coverage
- July 1, 2025. Added calculated Total Eligible Gifts & Donations field: created computed field that sums Lines 34000 and 34200, positioned after Total Ecological Gifts in both T1 extraction display and Tax Credits Analysis, implemented calculation logic in getFieldValue functions to handle CALC-34000+34200 field code for comprehensive charitable donation totals
...
- July 2, 2025. Implemented province-conditional provincial tax section display: all Ontario provincial tax extractions, credits sections, and tax bracket analyses are now conditional based on client's province of filing using getClientProvince() helper function, ensuring proper display only for provinces with provincial income tax while maintaining clean interface for non-provincial tax jurisdictions
- July 2, 2025. Implemented collapsed-by-default credit sections: all credit subcategories (Basic Credits, Employment Credits, Personal Situation Credits, Disability & Caregiver Credits, Education Credits, Medical Credits, Charitable Gifts & Donations, Ontario Non-Refundable Credits, and Refundable Credits) now start collapsed when opening the credits tab for cleaner initial display
- July 2, 2025. Renamed "Ontario Provincial Credits" to "Ontario Non-Refundable Credits": updated section title in both collapsed section header and blue summary box for more accurate terminology distinguishing from refundable provincial credits
- July 2, 2025. Corrected Ontario Medical Expenses line number: changed from Line 58689 to Line 58769 across ExtractedDataDisplay, T1FieldEditDialog, PDF parser mapping, and Python extractor for accurate Canadian tax form line number referencing
- July 2, 2025. Corrected Ontario Donations & Gifts line number: changed from Line 58729 to Line 58969 across ExtractedDataDisplay, T1FieldEditDialog, PDF parser mapping, and Python extractor for accurate Canadian tax form line number referencing
- July 2, 2025. Fixed Ontario Donations & Gifts extraction for small amounts: modified Python extractor minimum threshold logic to allow amounts < $1.00 for line 58969 (matching existing logic for line 34900), enabling extraction of amounts like $0.26 for accurate Ontario provincial donation credit tracking
- July 2, 2025. Removed minimum amount threshold from extraction logic: updated Python extractor to allow extraction of all amounts including those < $1.00 across all tax form fields for comprehensive data capture without false positive filtering
- July 2, 2025. Eliminated Line 58800 reference from Ontario credits summary: removed "Line 58800" text from bottom of Ontario Non-Refundable Credits section in blue summary box for cleaner display
- July 2, 2025. Added Tax Savings line under Ontario credits summary: included new "Tax Savings" section below Ontario Non-Refundable Credits displaying Line 61500 value with consistent blue styling
- July 2, 2025. Updated Ontario Tax Savings layout to match Federal format: changed Tax Savings from separate section to smaller text line below main amount, matching Federal Non-Refundable section styling with text-sm and text-blue-700 classes
...
- July 3, 2025. Updated T1 extract summary field label format: repositioned tax line references from right side back to field labels using format "Employment Income (Line 10100):" with gray color (#A3A3A3) for improved readability and consistent labeling across all income, deduction, credit, and tax sections
- July 3, 2025. Standardized spacing between subcategories across all tabs: converted all sections to use consistent space-y-4 spacing pattern matching the deductions tab, eliminating inconsistent mt-6 spacing in accounts tab sections for unified user interface spacing throughout Income, Deductions, Credits, and Accounts tabs
- July 3, 2025. Completed comprehensive tooltip system for all T1 line references: implemented FieldLabelWithTooltip component for all 125 T1 fields across Income, Deductions, Credits, Taxes, and Accounts sections, displaying "Field Name (Line XXXXX):" format with HelpCircle icon immediately following line references, created extensive tooltip mapping function with detailed explanations for 100+ T1 line numbers covering employment income, pension benefits, investment income, deductions, tax credits, and account fields, wrapped entire component with TooltipProvider for consistent tooltip functionality throughout the tax form interface
- July 3, 2025. Implemented comprehensive "Expand All" toggle functionality across all major tabs: added "Expand All"/"Collapse All" buttons to Income, Deductions, Credits, and Accounts tabs with smart state detection, each tab can expand/collapse all subsections simultaneously with tab-specific color theming (blue for Income/Credits, yellow for Deductions, purple for Accounts), unified toggle system supports 7 income sections, 4 deduction sections, 10 credit sections, and 7 account sections for enhanced user control over section visibility
- July 3, 2025. Fixed field alignment in all collapsible sections: added proper left padding (pl-6) to field items within both CollapsibleSection components and custom account sections so individual tax form fields and account details now align properly with their section titles instead of being flush left, improving visual hierarchy and readability across all Income, Deductions, Credits, and Accounts tabs with consistent indentation throughout the interface
- July 3, 2025. Unified Accounts tab styling with Income tab design: converted all account sections (RRSP/RRIF, TFSA, FHSA, RESP, RDSP, Capital Loss, AMT) from grid-based layout to field-row structure matching Income tab, replaced complex div structures with simple field-row classes, added FieldLabelWithTooltip components for tax form fields (lines 20800, 20805, 24600, 24630, 25200, 40427), maintained font-medium text-primary styling for consistent visual hierarchy across all tabs
- July 3, 2025. Updated T1 extract summary card colors to custom #E8E3F0 shade: replaced all blue styling (bg-blue-50, text-blue-700, text-blue-800, text-blue-600, border-blue-200, border-blue-300) with custom summary-card CSS classes using #E8E3F0 background and complementary text colors, applied to Income Sources, Non-Refundable Tax Credits, and Credits Summary sections for consistent custom branding
- July 3, 2025. Updated deductions section cards to custom #FBE8E3 shade: replaced all yellow styling (bg-yellow-50, text-yellow-700, text-yellow-800, text-yellow-600, border-yellow-200, border-yellow-300) with custom deductions-card CSS classes using #FBE8E3 background and complementary text colors, applied to Deductions from Income header and Deductions Summary sections for distinct section branding
- July 3, 2025. Updated credits section cards to custom #E5F0E3 shade: replaced summary-card styling with custom credits-card CSS classes using #E5F0E3 background and complementary text colors, applied to Non-Refundable Tax Credits header and Credits Summary sections (Federal/Provincial breakdown) for consistent green-themed section branding
- July 3, 2025. Updated taxes section cards to custom #F5EBDD shade: replaced green styling (bg-green-50, text-green-700, text-green-800, border-green-200) with custom taxes-card CSS classes using #F5EBDD background and complementary text colors, applied to Tax Summary header section for distinct warm beige section branding
- July 3, 2025. Updated accounts section cards to custom #DDEBEF shade: replaced purple styling (bg-purple-50, text-purple-700, text-purple-800, border-purple-200) with custom accounts-card CSS classes using #DDEBEF background and complementary text colors (#C8D3D9), applied to Investment & Savings Accounts header section for distinct soft blue-gray section branding
- July 3, 2025. Fixed Medical Credits section not expanding with "Expand All" button in credits tab: corrected creditSectionIds array to use 'education-medical-credits' instead of 'medical-credits' to match the actual CollapsibleSection ID, ensuring Medical Credits section properly responds to expand/collapse all functionality
- July 3, 2025. Fixed RRSP section not expanding with "Expand All" button in accounts tab: corrected accountSectionIds array to use 'rrsp-section' instead of 'rrsp-rrif-section' to match the actual toggleSection ID used in RRSP/RRIF section button, ensuring RRSP section properly responds to expand/collapse all functionality
- July 3, 2025. Added "Expand All" button to Tax Deductions Analysis section in household tax reports: implemented areAllDeductionSectionsExpanded() and toggleAllDeductionSections() functions, added expand/collapse button to section header with deductions-card-button styling, allows users to expand or collapse all deduction categories (Retirement Plan, Personal, Employment, Specialized) simultaneously for improved navigation
- July 3, 2025. Added "Expand All" button to Federal Tax Credits Analysis section in household tax reports: implemented areAllFederalCreditSectionsExpanded() and toggleAllFederalCreditSections() functions, added expand/collapse button to section header with credits-card-button styling, allows users to expand or collapse all federal credit categories (Basic Credits, Employment Credits, Personal Situation Credits, Disability & Caregiver Credits, Education Credits, Medical Credits, Charitable Gifts & Donations, Federal Refundable Credits) simultaneously for improved navigation
- July 3, 2025. Added "Expand All" button to Provincial Tax Credits Analysis section in household tax reports: implemented areAllProvincialCreditSectionsExpanded() and toggleAllProvincialCreditSections() functions, added expand/collapse button to section header with credits-card-button styling, allows users to expand or collapse all provincial credit categories (Ontario Non-Refundable Credits, Ontario Refundable Tax Credits) simultaneously for improved navigation
...
- July 2, 2025. Fixed Combined Tax Bracket Analysis table formatting in individual taxpayer reports: updated table styling to exactly match household report structure with current marginal tax bracket highlighting (gold border #D4B26A), right-aligned Income/Tax columns, proper total row styling, whole number formatting, and consistent background colors for improved visual consistency across all tax bracket analysis sections
- July 3, 2025. Completed comprehensive UI standardization across all analysis sections: implemented consistent circular checkmark/X mark styling with ✓ (green circles #88AA73) and ✗ (gold circles #D4B26A) across Tax Deductions Analysis, Federal Tax Credits Analysis, and Provincial Tax Credits Analysis sections, ensuring complete visual consistency with white text on colored circular backgrounds throughout all tax report analysis components
- July 2, 2025. Fixed Income Breakdown pie chart Net Income calculation in individual taxpayer reports: updated pie chart calculation to match Individual Financial Summary using detailed formula (Total Income - Total Tax - CPP Contributions - EI Premiums) instead of simplified formula, ensuring consistent Net Income values across pie chart visualization and financial summary reporting
- July 2, 2025. Fixed all Income Breakdown pie chart Net Income calculations in household tax reports: updated both household income breakdown pie chart and individual member pie charts to use detailed financial summary calculation (Total Income - Federal Tax - Provincial Tax - CPP Contributions - EI Premiums) instead of simplified formula, ensuring consistent Net Income values across all pie chart visualizations and financial summary reporting components
- July 2, 2025. Made all Tax Deductions Analysis subsections collapsible in household tax reports: added React useState for managing collapsed sections, imported ChevronDown and ChevronRight icons, implemented collapsible interface with border styling matching ExtractedDataDisplay format, all deduction categories (Retirement Plan, Personal, Employment, Specialized) now start collapsed with category totals displayed on header, expandable to show individual deduction items with checkmarks and amounts
- July 2, 2025. Made all Federal Tax Credits Analysis and Provincial Tax Credits Analysis subsections collapsible in household tax reports: added React state management for both Federal and Provincial credit sections, implemented collapsible interface for all Federal credit categories (Basic, Employment, Personal Situation, Disability & Caregiver, Education, Medical, Charitable Gifts & Donations, Refundable) and Provincial credit categories (Ontario Non-Refundable, Ontario Refundable), categories start collapsed showing only totals with expandable detailed views
- July 2, 2025. Standardized font size across all analysis sections: updated Tax Deductions Analysis category headers to use text-sm font size matching Federal Tax Credits Analysis and Provincial Tax Credits Analysis for consistent visual hierarchy across all collapsible sections
- July 2, 2025. Fixed collapsible sections default state: corrected Federal Tax Credits Analysis and Provincial Tax Credits Analysis section keys to include ampersand characters and "(Non-Refundable)" suffixes, ensuring all sections properly default to collapsed when opening tax reports
- July 2, 2025. Updated Provincial Tax Bracket Visualization with 2025 Ontario tax rates: replaced previous tax brackets with authentic 2025 rates including new income thresholds at $90,599 and $106,732, updated all four income types (Ordinary Income: 5.05%-13.16%, Capital Gains: 2.53%-6.58%, Eligible Dividends: -6.83% to 14.53%, Non-Eligible Dividends: 2.37%-20.17%) for accurate provincial tax visualization
...
- July 2, 2025. Completed comprehensive individual taxpayer reporting system: copied all individual analysis elements from household reports to individual reports including Combined Tax Bracket Analysis with authentic 2024 Ontario rates (20.05%-53.53%), Combined Tax Bracket Visualization with four income types (Ordinary Income, Capital Gains, Eligible Dividends, Non-Eligible Dividends), Federal Tax Bracket Analysis and Visualization with authentic federal rates (15%-33%), and Provincial Tax Bracket Analysis and Visualization with authentic Ontario rates (5.05%-13.16%), creating complete standalone individual taxpayer analysis capability with all tax bracket tables, visualizations, current bracket highlighting, and income indicators
...
- July 2, 2025. Created comprehensive individual taxpayer reporting system: built IndividualTaxReport component copying household report structure but focused on single taxpayer data, added route /household/:householdId/individual/:clientId/:year, implemented individual financial summary with pie chart, tax rates analysis, and KPI blocks showing personal "You Kept" and "You Paid" percentages, added purple report button (FileText icon) positioned after reprocess button in Tax Reports section for easy access to individual taxpayer analysis
```
